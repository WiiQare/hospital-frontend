...
const Transaction = () => {
  ...
  const handleClick = () => {
    ...
    const swalWithBootstrapButtons = Swal.mixin({
      customClass: {
        confirmButton:
          'bg-orange ml-6 py-3 px-4 rounded-lg uppercase font-semibold text-white',
    ...
    swalWithBootstrapButtons
      .fire({
        ...
        preConfirm: (redeem) => {
          ...
          return fetch(
            `https://api.wiiqare-app.com/api/v1/provider/redeem-voucher`,
            Options,
          )
            .then((response) => {
              if (!response.ok) {
  ...
  return (
    <div className="p-2 space-y-6 md:py-8 md:px-6 mb-20">
      ...
      <TableContext.Provider
        value={{ selected, setSelected, isChecked, setIsChecked, session }}
      >
        <TransactionTable />
      </TableContext.Provider>
    </div>
