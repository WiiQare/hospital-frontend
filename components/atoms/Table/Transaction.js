...
<<<<<<< HEAD
  ...
  },
  {
    name: 'HASH',
    selector: (row) => (
      <button className="btn btn-xs !lowercase">{row.shortenHash}</button>
    ),
    sortable: true,
  },
=======
	...
	{
		name: 'HASH',
		selector: row => (<button className="btn btn-xs !lowercase">{row.voucherEntity.shortenHash}</button>),
		sortable: true,
	},
>>>>>>> feature-refactoring-update-db-new-vouchers-table
];
