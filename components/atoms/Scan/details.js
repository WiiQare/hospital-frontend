...
const ScanDetails = ({shorten}) => {
    ...
    return (
        <div className='flex justify-center flex-col gap-6 h-full items-center mx-auto py-4 md:py-10 mb-20'>
			<div className='md:w-2/4 sm:w-2/3 w-full bg-white rounded-xl py-8 min-h-fit flex '>
				<div className="flex flex-col gap-6 justify-center w-full">
					...
					<ItemsDetails title={"Montant Pass Santé"} value={new Intl.NumberFormat("en-US", {style: 'currency', currency: data.currency}).format(data.amount)} otherValue={`${(data.amount - total) > 0 ?  'Reste: ' : 'Manquant: '}`  + new Intl.NumberFormat("fr-FR", {style: 'currency', currency: "CDF"}).format(data.amount - total)} />
