...
const ScanDetails = ({ shorten, country }) => {
  ...
  const handleCurrency = async (e) => {
    ...
    setDataAmount({
      amount: res.result,
      currency: res.query.to,
      rate: res.info.rate,
    });
    setDataAmountHospital({
      amount: resHospital.result,
      currency: resHospital.query.to,
      rate: resHospital.info.rate,
    });
    console.log(res);
  };
  ...
  if (!data)
    return (
      <div className="flex flex-col gap-3 items-center mt-14 h-full">
        <div
          className="animate-spin inline-block w-4 h-4 border-[2px] border-current border-t-transparent text-gray-700 rounded-full"
          role="status"
          ariaLabel="loading"
        >
    ...
    return (
      <div className="mx-auto flex justify-center items-center flex-col h-full gap-3">
        ...
        <button
          ...
          className="bg-orange text-white text-sm px-3 py-2 rounded-lg effect-up shadow-sm font-light"
        >
          Scannez à nouveau
        </button>
      </div>
      ...
      <div className="md:w-1/3 sm:w-2/3 w-full bg-white rounded-xl py-8 min-h-fit flex ">
        <div className="flex flex-col gap-6 justify-center w-full">
          ...
          <div className="flex gap-4 items-center px-5 justify-center">
            ...
            <img
              className="inline-block h-[2.875rem] w-[2.875rem] rounded-full ring-2 ring-white dark:ring-gray-800"
              src="https://i.goopics.net/9w9mvb.png"
              alt="Image Description"
            />
          </div>
          ...
          <ItemsDetails
            ...
            value={data.patientNames}
            exclamation={true}
          />
          <ItemsDetails
            title={'Hôpital'}
            value={session?.user?.data.names ?? session?.user?.data.name}
            exclamation={false}
          />
                        ...
                        <span className="text-xs text-gray-400">
                          ...
                          <span className="text-gray-800 font-bold">
                            {new Intl.NumberFormat('fr-FR', {
                              ...
                              dataAmount.amount - dataAmountHospital.amount,
                            )}
                          </span>
                        </span>
                      </div>
          ...
          <ItemsDetails
            title={'Montant Hopital'}
            value={new Intl.NumberFormat('fr-FR', {
              style: 'currency',
              currency: dataAmountHospital.currency,
            }).format(dataAmountHospital.amount)}
            otherValue={new Intl.DateTimeFormat('fr-FR', {
              dateStyle: 'medium',
            }).format(new Date())}
          />

          {dataAmount.amount - dataAmountHospital.amount < 0 ? (
            <span className="text-xs text-red-400 font-normal px-4">
              * Ce traitement coûte plus cher que le montant dans ce Pass Santé
            </span>
          ) : (
            <></>
          )}
