...
export async function sendSecurityCode (formData) {
    try {
        ...
        const response = await fetch(BASE_URL + "/provider/provider-authorize-voucher-transfer", Options);
        const json = await response.json();
...
export async function addPackage (formData) {
    try {
        ...
        const response = await fetch(BASE_URL + `/provider/${formData.providerId}/package`, Options);
        const json = await response.json();
